...
class Attention(nn.Module):
    ...
    def forward(self, x, register_hook=False, prompt=None, register_blk=None, ep_g=None, client_index=None):
        ...
        if prompt is not None:
            '''
            pk, pv = prompt
            k_prompt = torch.cat((pk, x), dim=1)
            v_prompt = torch.cat((pv, x), dim=1)
            qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            q = qkv[0]
        #q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)
            k_qkv = self.qkv(k_prompt).reshape(B, k_prompt.shape[1], 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            v_qkv = self.qkv(v_prompt).reshape(B, v_prompt.shape[1], 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            k = k_qkv[1]
            v = v_qkv[2]
            '''
        ...
        '''
        if ep_g is not None and (ep_g + 1) % 3 == 0:
            weight_matrix = torch.mean(attn.clone().reshape(-1, attn.shape[2], attn.shape[3]), dim=0).detach().cpu().numpy()
            fig, ax = plt.subplots()
            ...
            plt.yticks([])
            plt.colorbar(im)
            plt.savefig('/home/piaohongming/FCL/Baselines/src/Picture/Attention_map_pt/{}_{}_{}.png'.format(client_index, register_blk, ep_g), dpi=300)
            plt.close()
        '''
...
class VisionTransformer(nn.Module):
    ...
    def get_aqk(self, x, prompt=None, client_index = None, q=None, task_id=None, trained_task_id=None, finished_task=None):
        ...
        for i,blk in enumerate(self.blocks):
            if prompt is not None:
                p_list, loss, x, P_, indices_taskchoosing, mean_aqk = prompt.get_aqk(q, i, x, task_id=task_id, trained_task_id=trained_task_id, client_index=client_index, finished_task=finished_task)
    ...
    def forward(self, x, register_blk=-1, prompt=None, q=None, train=False, task_id=None, aq_k=None, ep_g=None, client_index=None):
        ...
        if isinstance(self.device, int):
            prompt_loss = torch.zeros((1,), requires_grad=True).cuda(self.device)
        else:
        ...
        if "classincremental" in self.args.method:
            for i,blk in enumerate(self.blocks):
                if prompt is not None:
                    p_list_divide, loss_divide, x_divide, P_divide_, indices_taskchoosing_divide, mean_aqk_divide = prompt.forward_divide(q, i, x_divide, train=True, task_id=task_id, aq_k=aq_k)
                else:
                    p_list_divide = None
                #print(x_divide.size())
                x_divide = blk(x_divide, register_blk=i, prompt=p_list_divide, ep_g=ep_g, client_index=client_index)    
            x_divide = self.norm(x_divide)
            if train and prompt is not None:
                with torch.no_grad():
                    for i,blk in enumerate(self.blocks):
                        if prompt is not None:
                            p_list_divide2, loss_divide2, x_divide2, P_divide_2, indices_taskchoosing_divide2, mean_aqk_divide2 = prompt.forward_divide(q, i, x_divide2, train=True, task_id=task_id, aq_k=aq_k)
                        else:
                            p_list_divide2 = None
                        x_divide2 = blk(x_divide2, register_blk=i, prompt=p_list_divide2, ep_g=ep_g, client_index=client_index)    
                    x_divide2 = self.norm(x_divide2)
                    prompt.task_embedding[prompt.global_task_id_real[prompt.task_id * prompt.num_clients + prompt.client_index], :] = torch.mean(x_divide2[:,3 * prompt.e_p_length,:], dim=0)
            if not train and prompt is not None:
                with torch.no_grad():
                    detect_task_id = nn.functional.normalize(x_divide[:,3 * prompt.e_p_length_2,:], dim=1)
                    task_embedding = nn.functional.normalize(prompt.task_embedding, dim=1)
                    #print(detect_task_id.size())
                    detect_task_id = torch.mm(detect_task_id, task_embedding.transpose(0,1))
                    if prompt.client_index == -1:
                        global_task_min_output = sorted(list(set(list(range(prompt.e_task_number))) - set(prompt.trained_task_id)))
                        #global_task_id = prompt.task_id * prompt.num_clients + prompt.client_index
                        #global_task_id = prompt.global_task_id_real[global_task_id]
                        #trained_task_id_removed = sorted(set(prompt.trained_task_id)-set([global_task_id]))
                        detect_task_id[:,global_task_min_output] = -float('inf')
                        detect_task_id = torch.max(detect_task_id, dim=1)[1].squeeze()
                        if len(detect_task_id.shape) == 0:
                            detect_task_id = detect_task_id.unsqueeze(0)
                    else:
                        client_task_min_output = sorted(list(set(list(range(prompt.e_task_number))) - set(prompt.client_learned_global_task_id)))
                        detect_task_id[:,client_task_min_output] = -float('inf')
                        detect_task_id = torch.max(detect_task_id, dim=1)[1].squeeze()
                        if len(detect_task_id.shape) == 0:
                            detect_task_id = detect_task_id.unsqueeze(0)
        if "classincremental" in self.args.method and not train:
            with torch.no_grad():
                x_out = None
                for sample_i in range(B):
                    sample_x = x[sample_i].unsqueeze(0)
                    for i,blk in enumerate(self.blocks):
                        if prompt is not None:
                            p_list, _, sample_x, indices_taskchoosing = prompt.forward_detect(q[sample_i].unsqueeze(0), i, sample_x, train=False, task_id=task_id, aq_k=aq_k, detect_task_id=detect_task_id[sample_i])
                        else:
                            p_list = None
                        sample_x = blk(sample_x, register_blk=i, prompt=p_list, ep_g=ep_g, client_index=client_index)
                    if x_out is None:
                        x_out = sample_x
                    else:
                        x_out = torch.cat((x_out, sample_x), dim=0)
                x = x_out
        else:
            for i,blk in enumerate(self.blocks):
        ...
        if "classincremental" in self.args.method:
            if prompt is not None:
                return x, prompt_loss, prompt_client, None, mean_aqk_list, feature_map, x_divide
            else:
                return x, prompt_loss, prompt_client, feature_map
        else:
            if prompt is not None:
                return x, prompt_loss, prompt_client, indices_taskchoosings[0], mean_aqk_list, feature_map
            else:
                return x, prompt_loss, prompt_client, feature_map

...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1, prompt=None, q=None, train=False, task_id=None):
        ...
        prompt_loss = torch.zeros((1,), requires_grad=True).cuda('cuda:2')
        prompt_client = []
        ...
        for i,blk in enumerate(self.blocks):
            ...
            if prompt is not None:
                #print("kkk")
                if train:
                    p_list, loss, x, P_ = prompt.forward(q, i, x, train=True, task_id=task_id)
                    prompt_loss += loss
        ...
        return x, prompt_loss, prompt_client
